...
async function init() {
    ...
    // --- ИЗМЕНЕНО: Логика инициализации звука ---
    const initAudio = () => {
        // Возобновляем аудиоконтекст, если он был приостановлен
        if (PIXI.sound.context.state === 'suspended') {
            PIXI.sound.context.resume();
        }
        // Создаем и загружаем звуки только после первого клика
        if (!audioManager) {
            audioManager = new AudioManager();
            audioManager.load();
        }
        // Убираем обработчик, чтобы он сработал только один раз
...
function handleBuyBonus() {
    ...
    if (playerBalance >= cost) {
        playerBalance -= cost;
        ui.updateBalance(playerBalance);
        if (audioManager) audioManager.play('buy_bonus_sound');
...
async function startSpin(isFirstFreeSpin = false) {
    ...
    if (audioManager) audioManager.play('spin_sound');
    ...
    while (true) {
        ...
        if (scatterCount >= config.freeSpins.triggerCount && !freeSpinsManager.isActive) {
            if (audioManager) audioManager.play('bonus_trigger_sound');
            freeSpinsManager.start(config.freeSpins.initialSpins);
            character.setPowerState(true);
        ...
        if (payout > 0 && audioManager) audioManager.play('win_sound');
    ...
    if (totalMultiplierOnScreen > 0 && currentSpinTotalWin > 0) {
        if (audioManager) audioManager.play('multiplier_apply_sound');
        if (freeSpinsManager.isActive) {
